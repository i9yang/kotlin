...
public class ESBundleCtgIdGroup extends ESCollection implements Groupable {
	@Override
	public String getCollectionAliasName(Parameter parameter) {
		return "ESBundleCtgIdGroup";
	}
	...
	public Result getResult(SearchResult searchResult, String name, Parameter parameter, Result result) {
		...
		if(StringUtils.equals(parameter.getUserInfo().getDeviceDivCd(), "20")) {
			result.setCategoryList(ResultUtils.getNewBrandCategoryGroup(parameter.getSiteNo(), parameter.getDispCtgId(), Joiner.on("@").join(ctgLst)));
		} else {
			result.setCategoryList(ResultUtils.getBrandCategoryGroup(parameter.getSiteNo(), parameter.getDispCtgId(), Joiner.on("@").join(ctgLst)));
		}
		return result;
	}
}
