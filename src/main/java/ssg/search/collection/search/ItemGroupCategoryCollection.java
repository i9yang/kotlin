 ...
 */
public class ItemGroupCategoryCollection extends ItemCategoryCollection implements Collection, CategoryGroupable, Prefixable {
	...
	private final Comparator<Category> itemCountComparator = new Comparator<Category>() {
		...
		public int compare(Category c1, Category c2) {
			if (c1.getRecomYn().equals("Y")){
				...
				return 1;
			} else if(c1.getThemeYn().equals("Y") && c2.getThemeYn().equals("N")){
				return 1;
			} else if(c2.getThemeYn().equals("Y") && c1.getThemeYn().equals("N")){
				return -1;
	...
	public Result getResult(Search search, String name, Parameter parameter, Result result) {
		...
		String[] idxs = strCtgIdxNm.split(",");
		List<Category> ctgList = Lists.newArrayList();
		for(int k=0;k<idxs.length;k++){
			// 통합검색의 경우 2개 뎁스로 작업을 진행한다. ( 타겟이 ALL 이고 CTG_ID, CTG_LV 값이 없는 경우 )
			if(( parameter.getTarget().equalsIgnoreCase("all") || parameter.getTarget().equalsIgnoreCase("category")) && StringUtils.isEmpty(parameter.getCtgId()) && StringUtils.isEmpty(parameter.getCtgLv())){
				// LEVEL 1 을 먼저 선행하고,
				int level = 1;
				int next  = 2;
				int count = search.w3GetCategoryCount(name, idxs[k], level);
				List<Category> ctgLv1List = this.getCategory(level, next, count, name, idxs[k], search);
				// LEVEL 2 를 실행
				level = 2;
				next  = 3;
				count = search.w3GetCategoryCount(name, idxs[k], level);
				List<Category> ctgLv2List = this.getCategory(level, next, count, name, idxs[k], search);
				// 결과가 정상이라면
			...
			// 카테고리 타겟으로 실행하는 경우
				...
				List<Category> ctgResultList = this.getCategory(level, next, count, name, idxs[k], search);
				...
				for(int i=0;i<search.w3GetCategoryCount(name, idxs[k], level);i++){
					String ctgInfo      = search.w3GetCategoryName(name, idxs[k], level, i);
