...
public abstract class TrieStringMatcher{
	...
	protected class TrieNode implements Comparable{
		 ...
		 * <code>true</code>, the new node is a <em>terminal</em> node in the
		 * trie.
		 */
		 ...
		 * <code>nextChar</code>. If no such node exists, one will be is added.
		 * If <em>isTerminal</em> is <code>true</code>, the node will be a
		 * terminal node in the trie.
		 */
		...
		TrieNode getChild(char nextChar){
			if(children == null) {
				children = (TrieNode[]) childrenList
						.toArray(new TrieNode[childrenList.size()]);
				childrenList = null;
	 ...
	 * <code>String</code> can be decoded and the last character is represented
	...
	/**
	 * Adds any necessary nodes to the trie so that the given
	 * <code>String</code> can be decoded <em>in reverse</em> and the first
	 * character is represented by a terminal node. Zero-length
	...
	/**
	 * Returns the shortest substring of <code>input<code> that is matched by a
	 * pattern in the trie, or <code>null<code> if no match exists.
	 */
	public abstract String shortestMatch(String input);

	/**
	 * Returns the longest substring of <code>input<code> that is matched by a
	 * pattern in the trie, or <code>null<code> if no match exists.
	 */
