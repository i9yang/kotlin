...
import ssg.front.search.api.collection.wisenut.bshop.WnBshopItemCategory
...
class WnItemGroupCategory : WnBshopItemCategory(), CategoryGroupable, Prefixable {
    private var lctgId = ""
    private var lctgNm = ""
    ...
    private val logger = LoggerFactory.getLogger(WnItemGroupCategory::class.java)
    private val itemCountComparator = Comparator<Category> { c1, c2 ->
        if (c1.recomYn == "Y") {
            -1
        } else if (c2.recomYn == "Y") {
            1
        } else if (c1.themeYn == "Y" && c2.themeYn == "N") {
            1
        } else if (c2.themeYn == "Y" && c1.themeYn == "N") {
            -1
    ...
    override fun getResult(search: Search, name: String, parameter: Parameter, result: ssg.front.search.api.dto.result.Result): ssg.front.search.api.dto.result.Result {
        ...
        if (strSiteNo == "6005") {
            ...
            if (parameter.ctgId.isNullOrEmpty()) {
                strCtgIdxNm = "DISP_CTG_IDX,TEM_DISP_CTG_IDX"
            } else {
                if (parameter.themeYn.isNullOrEmpty() || parameter.themeYn == "N") {
        ...
        val idxs = strCtgIdxNm.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        var ctgList: MutableList<Category> = arrayListOf<Category>()
        for (k in idxs.indices) {
            // 통합검색의 경우 2개 뎁스로 작업을 진행한다. ( 타겟이 ALL 이고 CTG_ID, CTG_LV 값이 없는 경우 )
            if ((parameter.target.equals("all", ignoreCase = true) || parameter.target.equals("category", ignoreCase = true)) && parameter.ctgId.isNullOrEmpty() && parameter.ctgLv.isNullOrEmpty()) {
                // LEVEL 1 을 먼저 선행하고,
                var level = 1
                var next = 2
                var count = search.w3GetCategoryCount(name, idxs[k], level)
                val ctgLv1List = this.getCategory(level, next, count, name, idxs[k], search)
                // LEVEL 2 를 실행
                level = 2
                next = 3
                count = search.w3GetCategoryCount(name, idxs[k], level)
                val ctgLv2List = this.getCategory(level, next, count, name, idxs[k], search)
                // 결과가 정상이라면
                if (ctgLv1List != null && ctgLv1List.size > 0) {
                    for (i in ctgLv1List.indices) {
                        ...
                        // childCtgList Sort
                        if (childCtgList != null && childCtgList.size > 0) childCtgList.sortedWith(itemCountComparator)
                    ...
                    // CtgList Sort
                    if (ctgList != null && ctgList.size > 0) ctgList.sortedWith(itemCountComparator)
                ...
                val count = search.w3GetCategoryCount(name, idxs[k], level)
                // 다다음 뎁스의 카테고리 정보를 이용해서 자식여부를 판별하기 위한 다음 뎁스 카테고리 아이디를 미리 매핑해둔다 ( 1, 2, 3 (자식여부Y) 레벨 클릭시에만 사용 )
                val ctgResultList = this.getCategory(level, next, count, name, idxs[k], search)
                ...
                if (level == 2) {
                    ...
                    for (c in ctgResultList) {
                        if (recomSctgIdSet != null && recomSctgIdSet.contains(c.getCtgId())) {
                            c.setRecomYn("Y")
                        }
                        ctgList!!.add(c)
                    }
                ...
                for (i in 0 until search.w3GetCategoryCount(name, idxs[k], level)) {
                    val ctgInfo = search.w3GetCategoryName(name, idxs[k], level, i)
